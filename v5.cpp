...
void computeSafeIntervals(GridMap& gm, const vector<DynamicObstacle>& obs) {
    ...
    const float PLANNING_HORIZON = 50.0f; // Daha uzun planlama ufku
    const float SAFETY_MARGIN = 0.1f; // Güvenlik marjı
...
SIPP_Result runSIPP(GridMap& gm, int sr, int sc, int gr, int gc, float cellSize) {
    ...
    if (goal_node) {
        ...
        cout << "Path oluşturuldu, " << path.size() << " nokta" << endl;
        for (size_t i = 0; i < path.size(); i++) {
            cout << "  " << i << ": (" << path[i].first.x << "," << path[i].first.y << ") at " << path[i].second << endl;
        }
    }
    else
    {
...
struct Agent {
    ...
    void update(float dt) {
        ...
        float t0 = path[segment].second;
        float t1 = path[segment + 1].second;
        float factor = (simTime - t0) / (t1 - t0);

        const auto& p0 = path[segment].first;
        const auto& p1 = path[segment + 1].first;

        // Pozisyonu güncelle
        sf::Vector2f newPos = p0 + factor * (p1 - p0);
        shape.setPosition(newPos);
        
        // Debug: Pozisyonu konsola yazdır
        if (static_cast<int>(simTime * 10) % 10 == 0) { // Her 0.1 saniyede bir
            cout << "Agent at: (" << newPos.x << ", " << newPos.y << ") at time " << simTime << endl;
        }
    }
