...
void getSuccessors(const shared_ptr<State>& s, GridMap& gm, vector<shared_ptr<State>>& successors, int gr, int gc) {
    ...
    for (int k = 0; k < 4; k++) {
        ...
        for (auto& succ_interval : gm.safe[nr][nc])
        {
            ...
            succ->g = s->g + moveCost; // Toplam maliyet = mevcut maliyet + hareket maliyeti
    ...
    for (auto& wait_interval : gm.safe[s->r][s->c])
    {
        ...
        if (wait_interval.start > s->time && wait_interval.start > s->interval.start)
        {
            ...
            wait_succ->g = s->g + (wait_interval.start - s->time); // Toplam maliyet = mevcut maliyet + bekleme süresi
...
struct Agent {
    ...
    void update(float dt, GridMap& gm, int goalR, int goalC, float cellSize, const vector<DynamicObstacle>& obstacles) {
        ...
        for (int i = 0; i < path.size() - 1; i++) {
            if (simTime >= path[i].second && simTime < path[i + 1].second) {
                ...
                // Mevcut hücre pozisyonunu güncelle
                int newCellR = static_cast<int>(pos.y / cellSize);
                int newCellC = static_cast<int>(pos.x / cellSize);

                // Hücre değiştiyse replan kontrolü yap
                if (newCellR != currentCellR || newCellC != currentCellC) {
                    currentCellR = newCellR;
                    currentCellC = newCellC;

                    // Her 2 saniyede bir replan yap
                    if (simTime - lastReplanTime > 2.0f) {
                        replan(gm, goalR, goalC, cellSize);
                    }
                }

                // Debug: Her 1 saniyede bir pozisyon yazdır
                static float lastDebugTime = 0;
                if (simTime - lastDebugTime > 1.0f) {
                    cout << "AGENT at time " << simTime << ": pos=(" << pos.x << "," << pos.y << ") cell=(" << currentCellR << "," << currentCellC << ") segment=" << i << endl;
    ...
    bool replan(GridMap& gm, int goalR, int goalC, float cellSize) {
        ...
        // SIPP ile yeni path bul
        SIPP_Result result = runSIPP(gm, currentCellR, currentCellC, goalR, goalC, cellSize);
...
int main() {
    ...
    while (window.isOpen()) {
        ...
        if (simulating) {
            ...
            // Update agent with replanning
            if (gm) {
                agent.update(dt, *gm, goalR, goalC, cellSize, obstacles);
            }
            if (!agent.active) {
